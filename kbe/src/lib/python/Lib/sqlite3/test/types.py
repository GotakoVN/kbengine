...
#
# Copyright (C) 2005 Gerhard H鋜ing <gh@ghaering.de>
#
...
class SqliteTypeTests(unittest.TestCase):
    ...
    def CheckString(self):
        self.cur.execute("insert into test(s) values (?)", ("謘terreich",))
        self.cur.execute("select s from test")
        row = self.cur.fetchone()
        self.assertEqual(row[0], "謘terreich")
    ...
    def CheckUnicodeExecute(self):
        self.cur.execute("select '謘terreich'")
        row = self.cur.fetchone()
        self.assertEqual(row[0], "謘terreich")
