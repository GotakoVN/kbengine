...
bool ClientSDKGodot::writeEntityDefsModuleBegin()
{
	...
	// init functions
	sourcefileBody_ += "func addDataTypeAlias(utype, aliasTypeName, realTypeName):\n";
	sourcefileBody_ += "\tdatatypes[aliasTypeName] = KBEngine.Helpers.tryGetValue(datatypes, realTypeName)\n";
	sourcefileBody_ += "\tid2datatypes[utype] = datatypes[aliasTypeName]\n";
	sourcefileBody_ += "\tdatatype2id[aliasTypeName] = utype\n\n";
	
	sourcefileBody_ += "func addCustomDataType(utype, typeName, typeInstance):\n";
	sourcefileBody_ += "\tdatatypes[typeName] = typeInstance\n";
	sourcefileBody_ += "\tid2datatypes[utype] = datatypes[typeName]\n";
	sourcefileBody_ += "\tdatatype2id[typeName] = utype\n\n";
...
{
	sourcefileBody_ += "func initDefTypes():\n";
	return true;
}
...
{
	sourcefileBody_ += "for dataType in datatypes.values():\n";
...
bool ClientSDKGodot::writeEntityDefsModuleInitDefType(const DataType* pDataType)
{
	...
	{
		sourcefileBody_ += fmt::format("\taddCustomDataType({}, '{}', KBEngine.CustomDataTypes.{}.new())\n", typeID, typeName, typeName);
	}
	...
	{
		sourcefileBody_ += fmt::format("\taddDataTypeAlias({}, '{}', '{}')\n", typeID, typeName, pDataType->getName());
	}
...
bool ClientSDKGodot::writeEntityCallMethodBegin(ScriptDefModule* pScriptDefModule, MethodDescription* pMethodDescription, const char* fillString1, const char* fillString2, COMPONENT_TYPE componentType)
{
	...
	for (; iter != argTypes.end(); ++iter)
	{
		...
		{
			writeName = fmt::format("KBEngine.EntityDef.id2datatypes[{}].addToStreamEx(bundle, arg{})",
...
bool ClientSDKGodot::writeBaseEntityCallBegin(ScriptDefModule* pScriptDefModule)
{
	...
	sourcefileBody_ += "class Base extends 'res://kbe_plugin/EntityCall.gd':\n";
	...
	if (!pScriptDefModule->isComponentModule())
	{
		...
		for (; propIter != clientPropertys.end(); ++propIter)
		{
			...
			sourcefileBody_ += fmt::format("\tvar {} = null\n", pPropertyDescription->getName());

			initstr += fmt::format("\t\t{} = KBEngine.Helpers.getBaseEntityCall('EntityCall{}{}').new({}, id)\n",
				pPropertyDescription->getName(), pEntityComponentType->pScriptDefModule()->getName(), moduleSuffix, pPropertyDescription->getUType());
		}

		sourcefileBody_ += "\n\tfunc _init(eid, ename).(eid, ename):\n";
		sourcefileBody_ += initstr;
	}
	else
	{
		sourcefileBody_ += fmt::format("\tvar entityComponentPropertyID = 0\n\n");

		sourcefileBody_ += fmt::format("\tfunc _init(ecpID, eid).(eid, '{}'):\n", pScriptDefModule->getName());
...
bool ClientSDKGodot::writeCellEntityCallBegin(ScriptDefModule* pScriptDefModule)
{
	...
	sourcefileBody_ += "class Cell extends 'res://kbe_plugin/EntityCall.gd':\n";
	...
	if (!pScriptDefModule->isComponentModule())
	{
		...
		for (; propIter != clientPropertys.end(); ++propIter)
		{
			...
			initstr += fmt::format("\t\t{} = KBEngine.Helpers.getCellEntityCall('EntityCall{}{}').new({}, id)\n",
				pPropertyDescription->getName(), pEntityComponentType->pScriptDefModule()->getName(), moduleSuffix, pPropertyDescription->getUType());
		}
